...
func _process(delta: float) -> void:
	...
	if rotation_delta > 0 && player.velocity.length() > 20.0:
		particle_spawn_angle = (Vector2.UP.rotated(player.player_rotation.rotation) + player_velocity_normalized).normalized()
		particle_spawn_angle.x *= -1.0
		particle_spawn_angle.y *= -1.0
	else:
		particle_spawn_angle = Vector2.UP.rotated(player.player_rotation.rotation) * Vector2(-1.0,-1.0)
